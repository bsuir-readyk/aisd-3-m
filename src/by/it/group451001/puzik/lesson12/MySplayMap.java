...
public class MySplayMap implements NavigableMap<Integer, String> {
    ...
    private static class Entry { Integer key; String value; Entry(Integer k,String v){key=k;value=v;} }
    private Entry[] data = new Entry[8];
    private int size = 0;

    @Override public String toString(){ StringBuilder out=new StringBuilder(); out.append('{'); for(int i=0;i<size;i++){ if(i>0) out.append(", "); out.append(data[i].key).append("=").append(data[i].value);} out.append('}'); return out.toString(); }
    ...
    @Override public String put(Integer key, String value){ int i=index(key); if(i>=0){ String old=data[i].value; data[i].value=value; return old;} int pos=-i-1; ensure(size+1); System.arraycopy(data,pos,data,pos+1,size-pos); data[pos]=new Entry(key,value); size++; return null; }
    @Override public String remove(Object key){ int i=index((Integer)key); if(i<0) return null; String old=data[i].value; System.arraycopy(data,i+1,data,i,size-i-1); data[--size]=null; return old; }
    ...
    @Override public Integer lowerKey(Integer key){ int i=lowerIndex(key,false); return i>=0? data[i].key:null; }
    @Override public Integer floorKey(Integer key){ int i=lowerIndex(key,true); return i>=0? data[i].key:null; }
    @Override public Integer ceilingKey(Integer key){ int i=higherIndex(key,true); return i<size? data[i].key:null; }
    @Override public Integer higherKey(Integer key){ int i=higherIndex(key,false); return i<size? data[i].key:null; }

    @Override public SortedMap<Integer,String> headMap(Integer toKey){ MyRbMap m=new MyRbMap(); for(int i=0;i<indexForInsert(toKey);i++) m.put(data[i].key,data[i].value); return m; }
    @Override public SortedMap<Integer,String> tailMap(Integer fromKey){ MyRbMap m=new MyRbMap(); for(int i=indexForInsert(fromKey);i<size;i++) m.put(data[i].key,data[i].value); return m; }
    @Override public SortedMap<Integer,String> subMap(Integer fromKey,Integer toKey){ MyRbMap m=new MyRbMap(); for(int i=indexForInsert(fromKey); i<indexForInsert(toKey); i++) m.put(data[i].key,data[i].value); return m; }
    ...
    // helpers
    private int index(Integer key){ int lo=0,hi=size-1; while(lo<=hi){ int mid=(lo+hi)>>>1; int cmp=Integer.compare(data[mid].key,key); if(cmp<0) lo=mid+1; else if(cmp>0) hi=mid-1; else return mid; } return -(lo+1); }
    private int indexForInsert(Integer key){ int idx=index(key); return idx>=0? idx: -idx-1; }
    private int lowerIndex(Integer key, boolean inclusive){ int i=indexForInsert(key); if(inclusive && i<size && Objects.equals(data[i].key,key)) return i; return i-1; }
    private int higherIndex(Integer key, boolean inclusive){ int i=indexForInsert(key); if(inclusive && i<size && Objects.equals(data[i].key,key)) return i; return i + (i<size && data[i].key<=key?1:0); }
